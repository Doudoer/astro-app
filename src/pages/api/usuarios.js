...
export async function GET({ request }){
  ...
  try{
    const [rows] = await conn.execute('SELECT id, usuario, role FROM usuarios ORDER BY id DESC');
...
export async function POST({ request }){
  ...
  try{
    ...
    try{
      const [res] = await conn.execute('INSERT INTO usuarios (usuario, password, role) VALUES (?, ?, ?)', [usuario, hashed, role || 'user']);
